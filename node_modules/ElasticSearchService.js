...
exports.ElasticSearchService = class ElasticSearchService {
    ...
    _generateMappingForStructure(structure) {
        ...
        Object.keys(structure).forEach(function(field){
            ...
            if (['string','autocomplete','csv','phone'].indexOf(type) !== -1 ) {
            ...
            if (type === 'csv') {
                mapping.properties[field]['type'] = 'string';
                mapping.properties[field]['index_analyzer'] = 'csv';
                mapping.properties[field]['search_analyzer'] = 'standard';
            }
    ...
    putDocument(indexName, doc, callback) {
        let docType = doc.type;
        let mappingType = String(docType).toLowerCase() + '_map_type';
        console.log("storing on es endpoint: ", "/" + indexName + "/" + mappingType + "/" + doc.id);

        try {

            //elastic complains if you store the _id, therefore we replace it with just id, to be able to search on it still
            let toStore = JSON.parse(doc.content || "{}");
            if (typeof toStore['_id'] !== 'undefined') {
                 delete toStore['_id'];
                 if (typeof toStore['id'] === 'undefined') {
                     toStore['id'] = doc.id;
                 }
            }

            this.execute('PUT', "/" + indexName + "/" + docType + "/" + doc.id, toStore, callback);

        } catch (ex) {
            callback({"message":'Unable to put document',"reason": ex.toString()},null);
        }
    }

    deleteDocument(indexName, doc, callback) {
        let docType = doc.type;
        let mappingType = String(docType).toLowerCase() + '_map_type';
        console.log("Deleting from es endpoint: ", "/" + indexName + "/" + mappingType + "/" + doc.id);
        try {
            this.execute('DELETE', "/" + indexName + "/" + docType + "/" + doc.id, null, callback);
        } catch (ex) {
            callback({"message":'Unable to put document',"reason": ex.toString()},null);
        }
    }
    ...
    _generateIndexDefinition(docType, structure) {
        ...
        if (hasCsv) {
            ...
            indexDef.settings['number_of_shards'] = 1;
            indexDef.settings['analysis']['analyzer']['csv'] = {
                type : 'pattern',
                pattern : ', ',
                lowercase : true
            };
    ...
    putAlias(aliasName, oldIndex, newIndex, callback) {
        ...
        this.execute("POST","/_aliases", actions, function (err, result) {
            console.log("PUT alias result",err,result);
            callback(err,result);
        });
